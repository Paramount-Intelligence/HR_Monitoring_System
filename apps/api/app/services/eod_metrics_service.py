"""Daily-scoped EOD metric calculation."""
...
from app.models.enums import AttendanceSessionStatus, TaskStatus, TimeLogStatus
...
def calculate_eod_metrics_for_user(
    ...
    for log in time_logs:
        ...
        minutes = _overlap_minutes(log.started_at, log_end, window_start, effective_end)
        if minutes > 0:
    ...
    if session:
        ...
        if logged_minutes == 0 and session_minutes > 0:
            logged_minutes = session_minutes
        elif (
            session.session_status == AttendanceSessionStatus.ACTIVE
            and session_minutes > logged_minutes
        ):
            logged_minutes = session_minutes

    assigned_tasks = db.query(Task).filter(Task.assigned_to == user.id).all()
    task_by_id = {task.id: task for task in assigned_tasks}
    ...
    for task in assigned_tasks:
        ...
        if completed_at and window_start <= _as_utc(completed_at) < window_end:
    ...
    logged_hours = round(logged_minutes / 60, 2)
    ...
    if session and session.check_out_at is None and session_minutes > 0:
        total_hours = round(session_minutes / 60, 2)
...
def apply_daily_metrics_to_report(report, metrics: EodDailyMetrics) -> None:
    ...
    report.work_mode = metrics.attendance.work_mode
